...
| **Architecture diagram** | [`docs/architecture.html`](smriti/docs/architecture.html) / [`docs/architecture.pdf`](smriti/docs/architecture.pdf) — standalone, one page |
| **Full project document** (detailed PDF) | [`docs/full-document.html`](smriti/docs/full-document.html) / [`docs/full-document.pdf`](smriti/docs/full-document.pdf) — the complete write-up: problem → architecture → agents → intake → compliance → corpus → evaluation → grounded decisions → roadmap |
...
> PDFs are pre-generated. To regenerate any of them: open the `.html` in Chrome → Print → *Save as PDF*.
